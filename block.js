...
const users = JSON.parse(localStorage.getItem('BlockedUsers'))
...
dataPanel.addEventListener('click', function OnPanelClicked(event) {
  ...
  } else if (event.target.matches('#remove-from-block-button')) {
...
function removeFromList(id) {
  // if block list is empty, end function
  if (!users) return
    ...
    rawHTML += `
      <div class="col-sm-2 mt-3" id="user-card">
        ...
        <div class="card-body">
          ...
          <button type="button" class="btn btn-danger" id="remove-from-block-button" data-id = ${item.id}>x</button>
        </div>
      </div>
    </div>
  </div>
